...
function getDtsType(type) {
    ...
    throw new Error('Unable to convert type to valid dts type')
}
...
function dtsFunction(name, params, returnType, {jsDocComment}){
    const func = dom.create.function(
        ...
        params.map(param => {
            ...
            const parameter = dom.create.parameter(param.name, getDtsType(param.type), parameterFlags)
...
function dtsProperty(property){
    ...
    const prop = dom.create.property(property.name, getDtsType(property.type), declarationFlags)
